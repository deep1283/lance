...
const DashboardSidebar: React.FC = () => {
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2 text-gray-500 hover:text-gray-300 hover:bg-[#1a1a1a] rounded-md transition-all duration-200 ${
              !(isExpanded || isMobileMenuOpen || isMobile) && "justify-center"
            }`}
            title={
              !(isExpanded || isMobileMenuOpen || isMobile) ? "Competitors" : ""
            }
          >
  ...
  return (
    <>
      ...
      {/* Overlay for mobile - only when menu is open */}
      {isMobileMenuOpen && (
        <div
      ...
      {/* Mobile Sidebar - Overlay when menu is open */}
      {isMobileMenuOpen && (
        <div className="fixed left-0 top-0 h-full w-64 bg-[#0a0a0a] border-r border-[#1f1f1f] z-40 lg:hidden">
